...
class RequestMessageFactoryTest {
    ...
    fun getRequestEventListChannel() {
        ...
        // Value from Factory
        val reqEventListExpectedObj = VeadoRequest.getEventList
        //Convert to JSON
    ...
    fun getRequestPayloadEventList() {
        ...
        // Value from Factory
        val paylEventListExpectedObj = VeadoRequest.getPayloadEventList
        // Convert to JSON
    ...
    fun getRequestPayloadEventPeek() {
        ...
        // Value from Factory
        val paylEventListExpectedObj = VeadoRequest.getPayloadEventPeek
        // Convert to JSON
        val paylEventListExpectedObjText = paylEventListExpectedObj.toJsonString()
    ...
    fun buildRequestEventListChannel() {
        ...
        val reqChannelListExpectedFactObj = VeadoRequest.getEventList
        //Convert to JSON
    ...
    fun buildPayloadEventList() {
        ...
        val payload1 = VeadoRequest.createPayload(event = PayloadEvent.LIST)
        ...
        // Should give the same List Payload - Should be same object as requestPayloadEventList
        val payload2 = VeadoRequest.createPayload(event = PayloadEvent.LIST, value = "BF")
    ...
    fun buildPayloadEventPeek() {
        ...
        // Should give the same List Payload - Should be same object as requestPayloadEventList
        val payload1 = VeadoRequest.createPayload(event = PayloadEvent.PEEK)
        val payload1Text = payload1.toJsonString()
        ...
        // Should give the same List Payload - Should be same object as requestPayloadEventList
        val payload2 = VeadoRequest.createPayload(event = PayloadEvent.PEEK, value = "BF")
        val payload2Text = payload2.toJsonString()
    ...
    fun buildPayloadSetState() {
        ...
        val payloadObj = VeadoRequest.createPayload(event = PayloadEvent.SET, value = "BF")
        val payloadText = payloadObj.toJsonString()

        assert(payloadText == payloadExpected)

        assertThrows<IllegalArgumentException> { VeadoRequest.createPayload(event = PayloadEvent.SET) }
    }

    @Test
    fun buildPayloadPushState() {
        val payloadExpected = """{"event":"push","state":"26"}"""
        val payloadObj = VeadoRequest.createPayload(event = PayloadEvent.PUSH, value = "26")
        val payloadText = payloadObj.toJsonString()

        assert(payloadText == payloadExpected)

        assertThrows<IllegalArgumentException> { VeadoRequest.createPayload(event = PayloadEvent.PUSH) }
    }

    @Test
    fun buildPayloadPopState() {
        val payloadExpected = """{"event":"pop","state":"26"}"""
        val payloadObj = VeadoRequest.createPayload(event = PayloadEvent.POP, value = "26")
        val payloadText = payloadObj.toJsonString()

        assert(payloadText == payloadExpected)

        assertThrows<IllegalArgumentException> { VeadoRequest.createPayload(event = PayloadEvent.POP) }
    }

    @Test
    fun buildPayloadEventStateThumb() {
        val payloadExpected = """{"event":"thumb","state":"3"}"""
        val payloadObj = VeadoRequest.createPayload(event = PayloadEvent.THUMB, value = "3")
        val payloadText = payloadObj.toJsonString()

        assert(payloadText == payloadExpected)

        assertThrows<IllegalArgumentException> { VeadoRequest.createPayload(event = PayloadEvent.THUMB) }
    }

    @Test
    fun buildPayloadListen() {
        val payloadExpected = """{"event":"listen","token":"mini.changestate"}"""
        val payloadObj = VeadoRequest.createPayload(event = PayloadEvent.LISTEN, value = "mini.changestate")
        val payloadText = payloadObj.toJsonString()

        assert(payloadText == payloadExpected)
        assertThrows<IllegalArgumentException> { VeadoRequest.createPayload(event = PayloadEvent.LISTEN) }
    ...
    fun buildPayloadUnlisten() {
        ...
        val payloadObj = VeadoRequest.createPayload(event = PayloadEvent.UNLISTEN, value = "mini.changestate")
        val payloadText = payloadObj.toJsonString()

        assert(payloadText == payloadExpected)

        assertThrows<IllegalArgumentException> { VeadoRequest.createPayload(event = PayloadEvent.UNLISTEN) }
    }
